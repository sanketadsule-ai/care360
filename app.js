  ...
  if (FB) {
    ...
    function buildCaseItemNode(c, idx) {
      ...
      if (c.channel === 'facebook') {
        ...
        author.innerHTML = INSTAGRAM_ICON;
      } else if (c.channel === 'twitter') {
        author.innerHTML = TWITTER_ICON;
      } else if (c.channel === 'google_play') {
    ...
    function renderCaseDetails(c) {
      ...
      else if (c.channel === 'instagram') name.innerHTML = INSTAGRAM_ICON;
    ...
    // ── Load real social data from the database (populated by n8n) ─────────
    // The app no longer connects to social platforms directly; it reads the
    ...
    async function loadFeedFromDB() {
      const list = document.getElementById('case-list');
      try {
        if (list && state.cases.length === 0) {
          list.innerHTML = '<div class="fb-status">Loading messages…</div>';
        }
        ...
        state.cases = (data.threads || []).map((t, i) => {
          ...
          return {
            id: t.id,
            source: platformLabel(t.platform),
            author: t.author,
            channel: t.platform,
        ...
        console.error('loadFeedFromDB failed:', err);
        if (list && state.cases.length === 0) {
          list.innerHTML = '<div class="fb-status">Could not load messages from the database.</div>';
        }
      }
    }

    // Load on startup, and refresh whenever the user opens the Inbox.
    loadFeedFromDB();
    const inboxSidebarItem = sidebar.querySelector('.sidebar-item[data-page="inbox"]');
    if (inboxSidebarItem) inboxSidebarItem.addEventListener('click', loadFeedFromDB);
