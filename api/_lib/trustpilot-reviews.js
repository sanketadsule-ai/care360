  ...
  try {
    ...
    if (req.method === 'GET') {
      ...
      let query = `
        ...
        WHERE c.organization_id = $1 AND c.platform = 'trustpilot' AND c.deleted_at IS NULL
      `;
      let params = [orgId];
        ...
        try {
          ...
          const convRes = await client.query(`
            ...
            ON CONFLICT (channel_id, platform_thread_id) DO UPDATE SET
              ...
              updated_at = NOW()
            RETURNING id
          `, [orgId, channel_id, uniqueId, heading, escalation.priority, escalation.next_action, escalation.department, escalation.user_type, receivedAt]);
